...
export default function App() {
  ...
  const pages = useMemo(() => {
    return kategoriproduk.data
      ? Math.ceil(kategoriproduk.data?.length / rowsPerPage)
      : 0;
  }, [kategoriproduk.data?.length, rowsPerPage]);

  // if (kategoriproduk.error) return <div>failed to load</div>;
  // if (kategoriproduk.isLoading) return <div>loading...</div>;

  const loadingState = kategoriproduk.isLoading === 0 ? "loading" : "idle";
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        <TableBody
          items={
            kategoriproduk.data
              ? kategoriproduk.data.slice(offset, offset + rowsPerPage)
              : []
          }
