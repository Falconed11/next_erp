...
export default function app() {
  ...
  const deleteButtonPress = async (id) => {
    if (confirm("Hapus kategoriproduk?")) {
      const res = await fetch(`${api_path}kategoriproduk`, {
        method: "DELETE",
        headers: {
  ...
  const renderCell = {
    kategoriproduk: React.useCallback((data, columnKey) => {
  ...
  return (
    <div>
      ...
      <Table className="pt-3" aria-label="Example table with custom cells">
        <TableHeader columns={col.kategoriproduk}>
          {(column) => (
            <TableColumn
              key={column.key}
        ...
        <TableBody items={kategoriproduk.data}>
          {(item) => (
            <TableRow key={item.id}>
              {(columnKey) => (
                <TableCell>
                  {renderCell.kategoriproduk(item, columnKey)}
                </TableCell>
              )}
            </TableRow>
          )}
      ...
      <Modal
        ...
        isOpen={modal.kategoriproduk.isOpen}
        onOpenChange={modal.kategoriproduk.onOpenChange}
      >
        <ModalContent>
          {(onClose) => (
            <>
